...
def get_details(url, index):
    ...
    title = "(Index: " + index + ") - " + \
        str(soup.find('div', id="title").string)
    ...
    # Total number of comment pages
    total_comments_pages = soup.find('div', class_='browse-coms')
    opt = ''
    total_time = 0
    if total_comments_pages != None:
        total_comments_pages = int(
            soup.find('div', class_='browse-coms').strong.string)
        print("\n%d comment pages (1 page = 25 comments (MAX))" %
              (total_comments_pages))
        temp = True
        pg_count = 0
        # If more than 3 comment pages are present, ask user what to do.
        if(total_comments_pages > 2):
            while(temp):
                opt = input(
                    "Fetch all pages? May take longer [y/n/display anyway[d]]: ")
                if opt == 'y' or opt == 'Y':
                    pg_count = 0
                    temp = False
                elif opt == 'n' or opt == 'N':
                    pg_inp = input("Number of pages to fetch comments from? [0 < n < %d]: " % (
                        total_comments_pages));
                    if pg_inp == '':
                        print("Bad Input")
                    else:
        ...
        # Since last page is already fetched. So start from (n-1)th page
        ...
        while(total_comments_pages > pg_count):
            ...
            print("Page " + str(total_comments_pages) +
                  " [%.2f sec]" % (end_time))
            raw = raw.content
    ...
    torrent_hash = soup.find('div', class_="download").a["href"].split('&')[
        0].split(':')[-1]
    ...
    begin_tags = "<!DOCTYPE html><html><head>" + charset + \
        "<meta http-equiv='Content-type'> <title>" + \
        title + "</title>" + style_tag + "</head><body>"
    end_tags = "</body></html>"
    ...
    f.write(begin_tags)
    f.write("<h2><u><a href=" + url + " target='_blank'>" +
            name + "</a></u></h2><br />")
    ...
    # Magnetic link[1]
    f.write("<div class='center'><a href=" + magnet +
            " target='_blank'>[Magnetic Link (Download)]</a></div><br />")
    ...
    # Magnetic link[2]
    f.write(end_tags)
    f.write("<br /><div class='center'><a href=" + magnet +
            " target='_blank'>[Magnetic Link (Download)]</a></div><br /><br />")
    f.close()
